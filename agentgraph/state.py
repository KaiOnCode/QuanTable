...
class AgentState(MessagesState):
    ...
    current_position_pct: Annotated[float, "当前持仓百分比，例如 20意为20%，范围为 0~100"]
    ...
    # ========== 第一阶段：三个并行分析师（可并行执行）==========
    market_report: Annotated[Optional[str], "市场分析师（技术面）的报告，基于价格和技术指标"] = ""
    news_report: Annotated[Optional[str], "新闻研究员的报告，基于新闻事件和情绪分析"] = ""
    fundamental_report: Annotated[Optional[str], "基本面分析师的报告，基于财务数据和长期估值"] = ""
    
    # ========== 第二阶段：风险分析师（依赖三个并行报告）==========
    risk_report: Annotated[Optional[str], "风险分析师的报告，基于持仓、风险限额和上游三个报告"] = ""
    
    # ========== 第三阶段：最终裁决（依赖所有报告）==========
    PM_report: Annotated[Optional[str], "PM Agent的最终裁决报告，综合所有分析师的建议"] = ""
    Action: Annotated[Optional[str], "PM Agent的最终裁决动作，只能为 BUY/HOLD/SELL 之一"] = ""
    Target_position_pct: Annotated[Optional[float], "PM Agent的最终裁决目标持仓百分比，例如 50意为50%，范围为 0~100"] = ""
