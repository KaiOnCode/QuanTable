...
function shortHash(value: string): string {
  return value.length > 12 ? `${value.slice(0, 12)}…` : value || "unavailable";
}
    ...
    <section className="space-y-6" aria-labelledby="backtest-result-title">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">Frozen strategy <span className="font-mono text-foreground">{frozenStrategyId}</span> · job <span className="font-mono text-foreground">{shortHash(job.id)}</span></p>
        </div>
      ...
      {result.warnings.length > 0 ? <Card><CardHeader><CardTitle className="flex items-center gap-2 text-base"><AlertTriangle /> Warnings and sample sufficiency</CardTitle><CardDescription>Warnings are returned by the completed job and remain visible with the result.</CardDescription></CardHeader><CardContent><ul className="space-y-2 text-sm text-muted-foreground">{result.warnings.map((warning) => <li key={warning} className="font-mono">{warning}</li>)}</ul></CardContent></Card> : null}
      ...
      <Card>
        ...
        <CardContent>
          <dl className="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2 xl:grid-cols-4">
            <div><dt className="text-xs text-muted-foreground">Execution timing</dt><dd className="mt-1">Signal at close, fill {config.execution_timing === "next_open" ? "next open" : "close bar"}</dd></div>
            ...
            <div><dt className="text-xs text-muted-foreground">Policy hash</dt><dd className="mt-1 font-mono text-xs">{shortHash(result.provenance.policy_hash)}</dd></div>
            <div><dt className="text-xs text-muted-foreground">Data hash</dt><dd className="mt-1 font-mono text-xs">{shortHash(result.provenance.data_snapshot_hash)}</dd></div>
          </dl>
        </CardContent>
      </Card>
