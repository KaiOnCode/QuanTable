...
class TaskTracker:
    ...
    def record(self, tool_name: str, ticker: str = "", status: str = "ok"):
        self.tools_called.append(ToolCallRecord(
            tool=tool_name, ticker=ticker, status=status,
            when=str(self.iterations)
        ))
        if ticker:
    ...
    def summary(self) -> str:
        ...
        ticker_str = ", ".join(sorted(self.tickers_seen)) if self.tickers_seen else "none"
