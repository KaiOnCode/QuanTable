...
def apply_tool_result_budget(
    messages: list[dict], max_chars: int = TOOL_RESULT_BUDGET
) -> list[dict]:
...
def micro_compact(messages: list[dict], keep_last: int = 3) -> list[dict]:
    ...
    result_indices = [
        (i, m)
        for i, m in enumerate(messages)
        if m.get("role") == "tool" and m.get("name", "") in MICROCOMPACT_WHITELIST
    ]
...
def llm_compress(
    ...
    try:
        ...
        summary = content if isinstance(content, str) else json.dumps(content)

        # Fix tool_call ↔ tool_result pairs
        compressed = _fix_tool_pairs(
            [{"role": "user", "content": f"[Conversation Summary]\n{summary}"}]
            + list(tail)
        )

        logger.info(
            "L3 compression: %d messages → %d messages + summary (%d chars)",
            len(messages),
            len(compressed),
            len(summary),
        )
