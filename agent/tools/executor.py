...
class StreamingToolExecutor:
    ...
    def has_unfinished(self) -> bool:
        ...
        with self._lock:
            return any(
                t.status in ("executing", "queued_write") for t in self._tools.values()
            )
    ...
    def get_remaining_results(self) -> dict[int, str]:
        ...
        with self._lock:
            read_tools = [
                (idx, t)
                for idx, t in self._tools.items()
                if t.status == "executing" and t.future
            ]
            write_tools = [
                (idx, t) for idx, t in self._tools.items() if t.status == "queued_write"
            ]
        ...
        for idx, tool in read_tools:
            if self._abort.is_set():
                ...
                self._results[idx] = tool.result or ""
            else:
                try:
                    if tool.future is None:
                        continue
                    tool.result = tool.future.result()
                    tool.status = "completed"
                except Exception as exc:
