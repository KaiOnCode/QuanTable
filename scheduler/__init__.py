...
class DataCollector:
    ...
    def _refresh_news(self) -> None:
        ...
        for ticker in active:
            try:
                ...
                pass
        logger.debug(
            "news refresh: %d/%d tickers with new articles",
            count,
            len(self._get_active_tickers()),
        )
    ...
    def _refresh_sentiment(self) -> None:
        ...
        logger.debug(
            "sentiment refresh: %d/%d tickers", count, len(self._get_active_tickers())
        )
    ...
    def _refresh_discovery(self) -> None:
        ...
        try:
            ...
            from agent.discovery import discover_related_tickers, gather_user_interests
            from dataflow.store import MarketDataStore
            from storage import get_store
            ...
            try:
                ...
                decision_tickers = list(
                    {d.get("ticker", "") for d in decisions if d.get("ticker")}
                )
            except Exception:
                pass
            ...
            # Call LLM
            suggestions = discover_related_tickers(
                interests, headlines, max_suggestions=5
            )
            if not suggestions:
                return
    ...
    def _execute(self, task: dict) -> None:
        ...
        def _run():
            try:
                ...
                logger.warning(
                    "MonitorRunner: execution failed for %s: %s",
                    task.get("name", ""),
                    exc,
                )
