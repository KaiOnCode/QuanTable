...
def get_price(
    ...
    end_date: Annotated[Optional[str], "结束日期（ISO格式，例如 2024-01-15T00:00:00Z），如果为None则使用当前日期"] = None,
) -> str:
    """
    获取指定股票代码的股价数据（OHLCV）。
    参数:
...
def get_fundamentals(
    symbol: Annotated[str, "公司的股票代码，例如 AAPL, TSM"],
    end_date: Annotated[Optional[str], "结束日期（ISO格式，例如 2024-01-15T00:00:00Z），如果为None则获取实时数据，否则获取历史数据"] = None,
) -> str:
    """
    获取指定股票代码的基本面数据。
    参数:
...
def get_news(
    ...
    """
    result = _data_service.df_get_news(symbol, window_days=window_days, end_date=end_date)
    ...
    for idx, news in enumerate(result, 1):
        ...
        # 格式化日期
        date_str = published_at[:10] if published_at and len(published_at) >= 10 else "未知日期"
