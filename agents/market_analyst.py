...
from agents.utils.agent_tools import get_price, get_indicators
...
def market_analyst_agent(llm):
    ...
    def run(state):
        ...
        if not messages:
            ...
            messages = [HumanMessage(content=f"请分析股票 {ticker} 的技术面，使用工具获取价格和技术指标数据，当前分析日期为 {date}，你的结论只能基于该日及之前的数据。")]
