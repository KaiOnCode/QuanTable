...
if "pytest" in sys.modules:
    ...
    pytest.skip(
        "comprehensive agent quality script requires a running API server",
    ...
    for i, line in enumerate(lines):
        if (
            ...
            try:
                ...
                if e == "tool_call":
                    ev["tools"].append(d.get("tool", "?"))
                elif e == "answer":
                    ev["answer"] = d.get("text", "")
            except Exception:
                pass
    return ev
...
def H(tool):
    return (f"tool_{tool}", lambda e: tool in e["tools"])


def N(tool):
    return (f"!tool_{tool}", lambda e: tool not in e["tools"])


def C(text):
    return (
        f"ans_{text[:20]}",
        lambda e: text.lower() in (e.get("answer") or "").lower(),
    )
...
def X(text):
    return (
        f"!ans_{text[:20]}",
        lambda e: text.lower() not in (e.get("answer") or "").lower(),
    )


def A():
    return ("answered", lambda e: len(e.get("answer") or "") > 5)
...
suites = [
    ...
    (
        ...
        [
            (
                "Analyze NVDA",
                "Analyze NVDA: price, technicals, news",
                [H("get_price"), A()],
            ),
