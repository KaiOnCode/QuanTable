...
try:
    ...
    from dataflow.providers.YFinance import df_get_prices, df_get_indicators, df_get_fundamentals
...
class Backtester:
    ...
    def run_historical_analysis(self, ticker: str, analysis_date: str, forward_days: int = 30):
        """运行历史分析"""
        from datetime import datetime, timedelta
...
def create_enhanced_chart(ticker: str, days: int = 90, show_sma=True, show_bollinger=False, 
    ...
    try:
        ...
        if show_sma and len(df) >= 20:
            ...
            fig.add_trace(go.Scatter(x=df['ts'], y=df['sma20'], name='SMA20', 
                                    line=dict(color='orange', width=1)), row=1, col=1)
            if len(df) >= 50:
                fig.add_trace(go.Scatter(x=df['ts'], y=df['sma50'], name='SMA50', 
        ...
        if show_rsi and len(df) >= 14:
            ...
            fig.add_trace(go.Scatter(x=df['ts'], y=df['rsi'], name='RSI', 
        ...
        if show_macd and len(df) >= 26:
            ...
            fig.add_trace(go.Scatter(x=df['ts'], y=df['macd'], name='MACD', 
                                    line=dict(color='blue')), row=current_row, col=1)
            fig.add_trace(go.Scatter(x=df['ts'], y=df['signal'], name='Signal', 
                                    line=dict(color='orange')), row=current_row, col=1)
            fig.add_trace(go.Bar(x=df['ts'], y=df['hist'], name='Histogram'), row=current_row, col=1)
            current_row += 1
        
        # 成交量
        if show_volume:
            colors = ['red' if df['c'].iloc[i] < df['o'].iloc[i] else 'green' 
                     for i in range(len(df))]
            fig.add_trace(go.Bar(x=df['ts'], y=df['v'], name='成交量', 
...
def parse_decision(report: str):
    ...
    for i, line in enumerate(lines):
        ...
        # 匹配 "方向:" 或 "方向："
        if ('方向' in line or 'Direction' in line.lower()) and (':' in line or '：' in line):
            # 提取冒号后的内容
        ...
        # 匹配 "时间范围:" 或 "时间范围："
        elif ('时间范围' in line or 'timeframe' in line.lower()) and (':' in line or '：' in line):
            separator = '：' if '：' in line else ':'
            parts = line.split(separator, 1)
            if len(parts) > 1:
                value = parts[1].strip()
        ...
        # 匹配 "一句话结论:" 或 "一句话结论："
        elif ('一句话结论' in line or '结论' in line or 'summary' in line.lower()) and (':' in line or '：' in line):
            separator = '：' if '：' in line else ':'
            parts = line.split(separator, 1)
            if len(parts) > 1:
                value = parts[1].strip()
                ...
                for j in range(i+1, min(i+3, len(lines))):
                    ...
                    if next_line and not any(keyword in next_line for keyword in ['方向', '时间范围', '置信度', '##', '###', '---']):
    ...
    if decision_info['direction'] == 'Unknown':
        ...
        # 推断方向
        if 'bullish' in report_lower or '看涨' in report_text or '买入' in report_text or 'buy' in report_lower:
            decision_info['direction'] = 'Bullish'
        elif 'bearish' in report_lower or '看跌' in report_text or '卖出' in report_text or 'sell' in report_lower:
            decision_info['direction'] = 'Bearish'
        elif 'neutral' in report_lower or '中性' in report_text or '持有' in report_text or 'hold' in report_lower:
        ...
        if decision_info['summary'] == 'Unknown':
            ...
            for sentence in sentences:
                ...
                if sentence and len(sentence) > 10 and len(sentence) < 150:
                    # 跳过包含字段名的句子
                    if not any(keyword in sentence for keyword in ['方向', '时间范围', '置信度', 'MARKET_REPORT', 'RISK_REPORT']):
...
def main():
    ...
    # Title
    st.markdown('<div class="main-header">📈 IntelliFin Assistant</div>', unsafe_allow_html=True)
    st.markdown('<div class="sub-header">Multi-Agent Based Intelligent Stock Analysis System</div>', unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        if mode == "Backtest Mode":
            ...
            st.caption(f"💡 The system will predict based on data from {backtest_date}, then validate actual performance for the next {forward_days} days.")
        ...
        if mode == "Real-time Analysis":
            ...
            analyze_button = st.button("🔄 Run Backtest", type="primary", use_container_width=True)
            ...
            if metrics:
                ...
                with col3:
                    ...
                    if macd_data:
                        ...
                        st.metric("MACD Signal", "Golden Cross" if signal_cross else "Normal")
                    else:
                ...
                if fundamental:
                    ...
                    with col4:
                        ...
                        st.metric("Rev Growth", f"{rev_growth:.1f}%" if rev_growth else "N/A")
        ...
        try:
            if mode == "Backtest Mode" and backtest_date:
                ...
                with st.status(f"🔄 Backtesting {ticker}...", expanded=True) as status:
                    ...
                    # Market Analyst
                    st.write("  📈 **Market Analyst** - Analyzing historical technicals...")
                    time.sleep(0.3)
                    ...
                    # Fundamental Analyst
                    st.write("")
                    st.write("  💼 **Fundamental Analyst** - Analyzing historical financials...")
                    time.sleep(0.3)
                    ...
                    result = backtester.run_historical_analysis(ticker, analysis_date_str, forward_days=forward_days)
                    ...
                    status.update(label=f"✅ {ticker} Backtest Completed!", state="complete", expanded=False)
                ...
                with st.status(f"🔄 Analyzing {ticker}...", expanded=True) as status:
                    ...
                    time.sleep(0.2)
                    st.write("    ├─ 📊 Calculating technical indicators (RSI, MACD, SMA)...")
                    time.sleep(0.2)
                    ...
                    result = agent.run(ticker, date=current_date, current_position_pct=0.0)
                    ...
                    st.write("")
                    st.write("🎯 **PM Agent** - Generating final investment decision...")
                    time.sleep(0.3)
                    ...
                    st.write("")
                    status.update(label=f"✅ {ticker} Analysis Completed!", state="complete", expanded=False)
    ...
    if st.session_state.get('analysis_result'):
        ...
        if result.get("is_backtest") and result.get("backtest_validation"):
            ...
            with col1:
                st.metric(
                    "Analysis Date Price",
                    f"${validation['analysis_price']:.2f}"
                )
            ...
            if pm_report:
                ...
                with col3:
                    ...
                    result_text = "✅ Predicted Accurate" if is_correct else "❌ Predicted Inaccurate"
                    st.markdown(f"""
                    <div class="metric-card">
                        <h4>Accuracy</h4>
...
"""
        ...
        if show_details:
            ...
            with st.expander("💼 Fundamental Analysis Report (Financial)", expanded=False):
                fundamental_report = result.get("fundamental_report", "no fundamental data")
        ...
        if news_sources:
            st.divider()
            with st.expander(f"📰 Reference News Sources ({len(news_sources)} items)", expanded=False):
                for idx, news in enumerate(news_sources, 1):
                    with st.container():
                        ...
                        with col1:
                            st.caption(f"📍 Source: {news.get('source', 'Unknown')} | 🕐 {news.get('published_at', '')[:10]}")
                        with col2:
                            url = news.get('url', '')
                            if url:
                                st.link_button("🔗 View Original", url, use_container_width=True)
    ...
    if st.session_state.analysis_history:
        st.divider()
        with st.expander(f"📜 Analysis History ({len(st.session_state.analysis_history)} items)", expanded=False):
            # 添加导出按钮
            col1, col2 = st.columns([3, 1])
            with col2:
            ...
            for idx, entry in enumerate(reversed(st.session_state.analysis_history)):
                with st.container():
                    ...
                    with col3:
                        mode_badge = "🔄 Backtest" if entry['mode'] == "Backtest Mode" else "📈 Real-time"
                        st.text(mode_badge)
                    ...
                    if pm_report:
                        decision_info = parse_decision(pm_report)
                        st.caption(f"Direction: {decision_info['direction']} | Timeframe: {decision_info['timeframe']}")
    ...
    # 页脚
    st.divider()
    st.caption("⚠️ Disclaimer: The analysis provided by this system is for reference only and does not constitute investment advice. Investing involves risks, and decisions should be made with caution.")
