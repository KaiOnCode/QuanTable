...
**Post-integration note:** A later review found `upstream/memory` rather than `upstream/feat/memory`. It is an old one-commit memory prototype, not a current `feat/*` line. Do not direct-merge it into `dev`; selectively port its service boundary and docs. See [memory-branch-analysis.md](memory-branch-analysis.md).
...
| `upstream/memory` | `5a52d9d` | Early trading-memory loop prototype: `MemoryService`, PM memory prompt context, memory docs, and old `agentgraph` hook. | Not a `feat/*` ref; one unique commit on an old baseline (`dev...upstream/memory = 130 1` after dev integration). | Prototype: valuable service/doc ideas, but current `dev` already has memory models/store/API/UI and the branch targets stale graph files. | Do not direct-merge; selectively port service boundary/docs and harden current quick_ask/server memory integration. |
