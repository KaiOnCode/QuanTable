    ...
    def _fetch(cat: str, ticker: str):
        try:
            ...
            return (
                cat,
                ticker,
                {"name": ticker, "price": None, "change_pct": None, "currency": "USD"},
            )
...
def run(hours: int = 0, progress_queue=None) -> dict:
    ...
    def _progress(stage: str, status: str, detail: str = ""):
        ...
        if progress_queue:
            progress_queue.put(
                ("progress", {"stage": stage, "status": status, "detail": detail})
            )
    ...
    _progress(
        "news",
        "done",
        f"{len(articles)} unique articles from {len(categorized)} categories",
    )

    elapsed_collect = time.time() - started
    logger.info(
        "MorningBrief: %d tickers, %d articles in %.1fs",
        t_count,
        len(articles),
        elapsed_collect,
    )
