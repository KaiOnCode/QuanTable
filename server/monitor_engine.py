...
def collect_news(monitor: dict, window_days: int = 7) -> list[dict]:
    ...
    if monitor_id and unique:
            ...
            normalized.append(
                {
                    ...
                    "source_name": a.get(
                        "source_name", a.get("source", a.get("_source", ""))
                    ),
        ...
        logger.info(
            "Monitor %s: %d new articles saved (sources: %s)",
            monitor_id,
            count,
...
def execute(monitor_id: str) -> dict:
    ...
    logger.info(
        "Monitor [%s] starting: mode=%s, keywords=%d, tickers=%d",
        name,
        mode,
        len(keywords),
        len(tickers),
    ...
    try:
        ...
        logger.info(
            "Monitor [%s] done: report=%s, news=%d, elapsed=%dms",
            name,
            rid,
            len(news_articles),
            report["raw_data"]["elapsed_ms"],
        )
    except Exception as exc:
